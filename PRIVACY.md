...
- Cached Lighthouse account and task summaries used by the popup and X task
  interface.
- Short-lived engagement capture state in `storage.session`.
...
## 3. Product experience tasks

Product experience tasks use Buyer review on the Lighthouse website. This extension no longer collects customer-site data, evaluates product rules, or generates product proofs.
...
- Host access for `x.com` and `twitter.com`: renders and verifies Lighthouse X
  engagement tasks.
- Host access for `www.binance.com`: supports Binance Square engagement tasks.
...
Browser-session data is cleared by the browser. Server-side campaign and verification records
