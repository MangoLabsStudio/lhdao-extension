...
import { getOrCreateDevicePublicKeyJwk } from '@/lib/device-key'
import {
  type CapturedAction,
...
import {
  ...
  type ProductExperienceRule,
  type ProductExperienceRuleMatch,
  randomProductExperienceNonce,
...
import {
  ...
  type CreateExtensionPairingLegacyVars,
  ...
  type MintProductExperienceTestTicketResult,
  type MintProductExperienceTicketResult,
  ...
  type SubmitProductExperienceProofResult,
...
import {
  ...
  localStore,
  type ProductExperienceTaskCache,
  type ProductExperienceTicketKind,
...
import { getDeviceId } from '@/lib/watermark'
import type {
  MsgResponse,
  PairingState,
  SubmitErrorCode,
...
export default defineBackground(() => {
  ...
  onMessage(async (req): Promise<MsgResponse> => {
    ...
    if (req.type === 'get-product-experience-public-state') {
      return {
        type: 'product-experience-public-state',
        state: await getProductExperiencePublicState(req.campaignId),
      }
    }
    if (req.type === 'save-product-experience-task') {
      await saveProductExperienceTask(req.task)
      return { type: 'product-experience-save-result', ok: true }
    }
    if (req.type === 'get-product-experience-task-for-url') {
      return {
        type: 'product-experience-task-for-url',
        task: await getProductExperienceTaskForUrl(req.href),
      }
    }
    if (req.type === 'submit-product-experience-matches') {
      return submitProductExperienceMatches(req.campaignId, req.ruleMatches)
    }
    return { type: 'ack' }
  })
...
// ── Product Report L2 product experience bridge ──────────────────────

async function getProductExperiencePublicState(campaignId: string) {
  const task = await readProductExperienceTask(campaignId)
  ...
  const expired = isProductExperienceTaskExpired(task)
  return {
    campaignId,
    status: expired ? ('expired' as const) : task.status,
    matchedRuleIds: task.matchedRuleIds,
    totalRuleCount: task.rules.length,
    authorizationRequired: false,
    currentOriginAllowed: true,
    version,
    capabilities: [PRODUCT_EXPERIENCE_CAPABILITY],
    error: expired ? ('SESSION_EXPIRED' as const) : task.lastError,
  }
}

async function saveProductExperienceTask(task: {
  campaignId: string
  ...
  const ticket =
    task.ticketKind === 'TEST'
      ? (
          await gql<MintProductExperienceTestTicketResult>(
            MINT_PRODUCT_EXPERIENCE_TEST_TICKET_MUTATION,
            { campaignId: task.campaignId },
          )
        ).mintProductExperienceTestTicket
      : (
          await gql<MintProductExperienceTicketResult>(
            MINT_PRODUCT_EXPERIENCE_TICKET_MUTATION,
            { campaignId: task.campaignId },
          )
        ).mintProductExperienceTicket

  if (ticket.ruleSetVersion !== task.configVersion) {
    throw new Error('VERSION_MISMATCH')
  }

  const cache: ProductExperienceTaskCache = {
    campaignId: task.campaignId,
    ticketKind: task.ticketKind,
    title: task.title,
    configVersion: task.configVersion,
    savedAt: task.savedAt,
    ticket: ticket.ticket,
    macKey: ticket.macKey,
    expiresAt: ticket.expiresAt,
    ruleSetVersion: ticket.ruleSetVersion,
    allowedOrigins: ticket.allowedOrigins.map(
      (origin) => new URL(origin).origin,
    ),
    completionMode: ticket.completionMode,
    rules: normalizeProductExperienceRules(ticket.rules),
    status: 'ready',
    matchedRuleIds: [],
    lastError: null,
    verifiedAt: null,
  ...
  await writeProductExperienceTask(cache)
  await broadcastProductExperienceStateChanged(task.campaignId)
}

async function getProductExperienceTaskForUrl(
  href: string,
): Promise<MsgProductExperienceTaskForUrl | null> {
  let origin: string
  try {
  ...
  const tasks = await readProductExperienceTasks()
  const candidates = Object.values(tasks)
    .filter((task) => !isProductExperienceTaskExpired(task))
    .filter((task) => task.status !== 'verified')
    .filter((task) => task.allowedOrigins.includes(origin))
    .sort((left, right) => right.savedAt - left.savedAt)
  const task = candidates[0]
  if (!task) return null
  if (task.status === 'ready') {
    await writeProductExperienceTask({ ...task, status: 'observing' })
    await broadcastProductExperienceStateChanged(task.campaignId)
  }
  return publicProductExperienceTask(task)
}

async function submitProductExperienceMatches(
  campaignId: string,
  ...
  if (isProductExperienceTaskExpired(task)) {
    await writeProductExperienceTask({
      ...task,
      status: 'expired',
  ...
  if (!allProductExperienceRulesMatched(task, ruleMatches)) {
    await writeProductExperienceTask({
      ...task,
      status: 'observing',
  ...
  const submitting: ProductExperienceTaskCache = {
    ...task,
    status: 'submitting',
    matchedRuleIds: ruleMatches.map((match) => match.ruleId),
    lastError: null,
  }
  await writeProductExperienceTask(submitting)
  await broadcastProductExperienceStateChanged(task.campaignId)

  try {
    const ts = Math.floor(Date.now() / 1000)
    const nonce = randomProductExperienceNonce()
    const canonical = buildProductExperienceCanonical({
    ...
    const sig = await hmacSignProductExperienceProof(task.macKey, canonical)
    const res = await gql<SubmitProductExperienceProofResult>(
      SUBMIT_PRODUCT_EXPERIENCE_PROOF_MUTATION,
      {
        input: {
          version: PRODUCT_EXPERIENCE_PROOF_VERSION,
    ...
    const result = res.submitProductExperienceProof
    if (!result.accepted) {
      await writeProductExperienceTask({
        ...submitting,
        status: 'error',
    ...
    return {
      type: 'product-experience-submit-result',
      ok: false,
      code: error instanceof Error ? error.message : 'VERIFICATION_FAILED',
    }
  }
}

type MsgProductExperienceTaskForUrl = Extract<
  MsgResponse,
  { type: 'product-experience-task-for-url' }
>['task']

function publicProductExperienceTask(
  task: ProductExperienceTaskCache,
): MsgProductExperienceTaskForUrl {
  return {
    campaignId: task.campaignId,
    ticketKind: task.ticketKind,
    configVersion: task.configVersion,
...
function allProductExperienceRulesMatched(
  task: ProductExperienceTaskCache,
  ruleMatches: ProductExperienceRuleMatch[],
): boolean {
  if (ruleMatches.length !== task.rules.length) return false
  const expected = new Set(task.rules.map((rule) => rule.id))
  const seen = new Set<string>()
  for (const match of ruleMatches) {
    if (!expected.has(match.ruleId) || seen.has(match.ruleId)) return false
    if (!task.allowedOrigins.includes(match.origin)) return false
    seen.add(match.ruleId)
  }
  return seen.size === expected.size
}

function normalizeProductExperienceRules(
  rules: {
    id: string
    title: string
    urlPattern: string
  ...
  return rules.map((rule) => {
    ...
    if (rule.condition.type === 'ELEMENT_EXISTS') {
      return { ...base, condition: { type: 'ELEMENT_EXISTS' } }
    }
    if (
      rule.condition.type === 'TEXT_CONTAINS' &&
      typeof rule.condition.expected === 'string'
    ) {
      return {
        ...base,
        condition: {
    ...
    if (
      rule.condition.type === 'ATTRIBUTE_EQUALS' &&
      typeof rule.condition.attributeName === 'string' &&
      typeof rule.condition.expected === 'string'
    ) {
      return {
        ...base,
        condition: {
          ...
          attributeName: rule.condition.attributeName,
          expected: rule.condition.expected,
        },
      }
    }
    if (
      rule.condition.type === 'COUNT_AT_LEAST' &&
      Number.isSafeInteger(rule.condition.minimumCount)
    ) {
      return {
        ...base,
        condition: {
          type: 'COUNT_AT_LEAST',
          minimumCount: rule.condition.minimumCount as number,
        },
      }
    }
    throw new Error('PRODUCT_EXPERIENCE_RULE_INVALID')
  })
}

async function readProductExperienceTasks(): Promise<
  Record<string, ProductExperienceTaskCache>
> {
  return (await sessionStore.get('productExperienceTasks')) ?? {}
}

async function readProductExperienceTask(
  campaignId: string,
): Promise<ProductExperienceTaskCache | null> {
  const tasks = await readProductExperienceTasks()
  return tasks[campaignId] ?? null
}

async function writeProductExperienceTask(
  task: ProductExperienceTaskCache,
): Promise<void> {
  const tasks = await readProductExperienceTasks()
  await sessionStore.set('productExperienceTasks', {
    ...tasks,
    [task.campaignId]: task,
  })
}

function isProductExperienceTaskExpired(task: ProductExperienceTaskCache) {
  const expiresAt = new Date(task.expiresAt).getTime()
  return Number.isFinite(expiresAt) && expiresAt <= Date.now()
}

async function broadcastProductExperienceStateChanged(
  campaignId: string,
): Promise<void> {
  const webUrl = `${WEB_ENDPOINT.replace(/\/+$/, '')}/*`
  try {
...
> {
  ...
  try {
    ;[deviceId, publicKeyJwk] = await Promise.all([
      getDeviceId(),
      getOrCreateDevicePublicKeyJwk(),
    ])
  } catch (e) {
    const reason = `Failed to prepare device identity: ${(e as Error).message}`
  ...
  for (let i = 0; i < 3; i++) {
    ...
    try {
      await gql<CreateExtensionPairingResult, CreateExtensionPairingVars>(
        CREATE_EXTENSION_PAIRING_MUTATION,
        createExtensionPairingVariables(code, deviceId, publicKeyJwk),
        { anonymous: true },
      )
      createOk = true
      break
    } catch (e) {
      if (isCreateExtensionPairingDeviceBindingUnsupported(e)) {
        try {
          await gql<
            CreateExtensionPairingResult,
            CreateExtensionPairingLegacyVars
          >(
            CREATE_EXTENSION_PAIRING_LEGACY_MUTATION,
            createExtensionPairingLegacyVariables(code),
            { anonymous: true },
          )
          createOk = true
          break
          ...
          const legacyMsg =
            legacyError instanceof Error
              ? legacyError.message
              : String(legacyError)
          if (/already in use/i.test(legacyMsg)) continue
          const reason = legacyMsg
          setPairingState({ kind: 'error', reason })
          scheduleReset()
          return { ok: false, reason }
        }
